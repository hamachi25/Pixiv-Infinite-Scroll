        ...
        function following_process() {
            ...
            function createElement(
                ...
                if (borderCount == 1 && showDividingLine) {
                    ...
                    `;
                    document
                        .querySelector(".sc-1y4z60g-4")
                        .insertAdjacentHTML("beforeend", borderElement);
                }
                ...
                if (matches[3]) {
                    ...
                    const fetchData = async () => {
                        ...
                        const url2 = `https://www.pixiv.net/ajax/user/${
                            matches[1]
                        }/profile/illusts?${sliceIllustId.join(
                            "&"
                        )}&work_category=illust&is_first_page=0`;
                    ...
                    const pageCountObserver = new IntersectionObserver((entries) => {
                        if (entries[0].isIntersecting) {
                            ...
                            if ((matches || matches2) && isValid) {
                                const url =
                                    Number(saveScrollPageCount) + Number(pageCountNumber) - 1;
                                newUrl = currentUrl.replace(/(p=)[^&]+/, "$1" + url);
                            } else {
                                if (currentUrl.includes("p=")) {
                                    const url =
                                        Number(saveScrollPageCount) + Number(pageCountNumber);
                                    newUrl = currentUrl.replace(/(p=)[^&]+/, "$1" + url);
                                } else {
                                    const separate = currentUrl.includes("?") ? "&" : "?";
